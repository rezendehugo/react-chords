...
const getNeckHorizonalLine = (pos, strings) =>
  `M ${offsets[strings].x} ${fretHeight * pos} H ${offsets[strings].length}`

const getNeckVerticalLine = (pos, strings, fretsOnChord) =>
  `M ${offsets[strings].y + pos * 10} 0 V ${fretHeight * fretsOnChord}`
...
const getNeckPath = (strings, fretsOnChord) =>
  Array.apply(null, Array(fretsOnChord + 1)).map((_, pos) => getNeckHorizonalLine(pos, strings)).join(' ').concat(
    Array.apply(null, Array(strings)).map((_, pos) => getNeckVerticalLine(pos, strings, fretsOnChord)).join(' '))
    ...
    <g>
      ...
      {baseFret === 1
        ? <path
          ...
          />
        : <text fontSize='0.25rem' fill='#444' fontFamily='Verdana' x={getBarreOffset(strings, frets, baseFret, capo)} y='8'>{baseFret}fr</text>}
      {!lite &&
        <g>
